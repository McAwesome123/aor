...
void LKGameWindow::load() {
	...
	if (magic != SAVE_MAGIC_NUMBER) {
		InfoMessage::get_message(InfoMessage::InfoMessageList::save_damaged).show_message(false);
		exit(0);
	}

	AorUInt mv = IO::read_uint(&m_save_file);
	if (mv != AOR_MAJOR_VERSION) {
		InfoMessage::get_message(InfoMessage::InfoMessageList::save_incompatible).show_message(false);
		exit(0);
	}
...
void LKGameWindow::closeEvent(QCloseEvent *event) {
	if (InfoMessage::get_message(InfoMessage::InfoMessageList::quit).show_message(false) == QMessageBox::Yes) {
		save();
		event->accept();
	} else {
		event->ignore();
